...
function init() {
	...
	if ( ! container ) {
		throw new Error(
			'Lacking element with ID: ' + ampAppShell.contentElementId
		);
	}
...
function loadUrl( url, { scrollIntoView = false, pushState = true } = {} ) {
	...
	fetchShadowDocResponse( url )
		.then( async ( response ) => {
			 ...
			 */
			currentShadowDoc = AMP.attachShadowDocAsStream(
				newContainer,
				String( url ),
				{}
			);
			...
			currentShadowDoc.ampdoc.whenReady().then( () => {
				...
				document.body.className = newContainer.shadowRoot.querySelector(
					'body'
				).className;
				...
				newContainer.shadowRoot.addEventListener(
					'click',
					handleClick
				);
				newContainer.shadowRoot.addEventListener(
				 ...
				 */
				const originalAdminBar = document.getElementById(
					'wpadminbar'
				);
				if ( originalAdminBar ) {
		...
		for ( const className of link.parentElement.classList ) {
			if ( className.startsWith( menuItemObjectNamePrefix ) ) {
				menuItemObjectName = className.substr(
					menuItemObjectNamePrefix.length
				);
				break;
			}
		}
			...
			if ( 0 === depth ) {
				...
				if ( menuItemObjectName ) {
					link.parentElement.classList.add(
						`current_${ menuItemObjectName }_item`
					);
				}
			} else if ( 1 === depth ) {
				...
				if ( menuItemObjectName ) {
					link.parentElement.classList.add(
						`current_${ menuItemObjectName }_parent`
					);
					link.parentElement.classList.add(
						`current_${ menuItemObjectName }_ancestor`
					);
				}
			} else {
				item.classList.add( 'current-menu-ancestor' );
